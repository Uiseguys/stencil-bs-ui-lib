...
export class CwcInlineedit {
    ...
    componentDidLoad() {
        ...
        let previousTimeout;
        this.inputWrapper.querySelector('textarea').addEventListener('focusout', () => {
        ...
        this.inputWrapper.querySelector('textarea').addEventListener('textarea', () => { 
            self.onchange.emit(self.inputWrapper.querySelector('textarea').value);
        });  
    }

    handleFieldClick() {
        const inputEl: any = this.inputWrapper.querySelector('textarea');
    ...
    handleCancelClick() {
        ...
        this.oncancel.emit();
        if (this.inputWrapper.querySelector('textarea').value !== this.fieldWrapper.innerText) {
            this.inputWrapper.querySelector('textarea').value = this.fieldWrapper.innerText;
            this.onchange.emit(this.inputWrapper.querySelector('textarea').value);
        }
    }     

    private saveInputText() {
        const value = this.inputWrapper.querySelector('textarea').value;
