...
export class CwcInlineedit {
  ...
  componentDidLoad() {
    ...
    this.inputWrapper.querySelector('input').addEventListener('input', () => {
      self.onChange.emit({
        id: this.id,
        value: self.inputWrapper.querySelector('input').value
      });
    });
  }
  ...
  handleCancelClick() {
    ...
    if (this.inputWrapper.querySelector('input').value !== this.fieldWrapper.innerText) {
      ...
      this.onChange.emit({
        id: this.id,
        value: this.inputWrapper.querySelector('input').value
      });
    }
  }
    ...
    return (
      <div>
        <div class="form-group">
          ...
          <div class="form-control field-wrapper" onClick={ () => this.handleFieldClick()}>{this.value}</div>
          <div class="input-wrapper">
            <input id={this.id} type="text" class="form-control" placeholder={this.placeholder} />
            <div class="button-wrapper">
              <button type="button" class="btn btn-light btn-sm" onClick={() => this.handleOkClick()}>
                <span><svg width="16" height="16" viewBox="0 0 24 24"><path d="M6.735 12.322a1 1 0 0 0-1.47 1.356l3.612 3.919c.537.526 1.337.526 1.834.03l.364-.359a2335.638 2335.638 0 0 0 3.939-3.883l.04-.04a492.598 492.598 0 0 0 3.658-3.643 1 1 0 0 0-1.424-1.404 518.42 518.42 0 0 1-3.64 3.625l-.04.04a2049.114 2049.114 0 0 1-3.775 3.722l-3.098-3.363z" fill="currentColor"/></svg></span>
              </button>
              <button type="button" class="btn btn-light btn-sm" onClick={() => this.handleCancelClick()}>
                <span><svg width="16" height="16" viewBox="0 0 24 24"><path d="M12 10.586L6.707 5.293a1 1 0 0 0-1.414 1.414L10.586 12l-5.293 5.293a1 1 0 0 0 1.414 1.414L12 13.414l5.293 5.293a1 1 0 0 0 1.414-1.414L13.414 12l5.293-5.293a1 1 0 1 0-1.414-1.414L12 10.586z" fill="currentColor"/></svg></span>
              </button>
            </div>
          </div>
        </div>
