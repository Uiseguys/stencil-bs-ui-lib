...
export class CwcInfiniteList {
  ...
  customEventHandler(event) {
    ...
    // if (event.detail === 'list-in-container') {
    //     // this.initUsers2Data()
    //     this.loadMoreUsers(10)
    // }
  }

  loadMoreUsers(count?: number): void {
    console.log("MOAR");

    this.getUsers(count).then(users => {
      console.log(users);
  ...
  getUsers(count = 10) {
    return new Promise(resolve => {
      ...
      request.open(
        "GET",
        `https://randomuser.me/api/?page=${this.getUsersPage()}&results=${count}&seed=abc`,
        true
      );
      request.onload = () => {
...
<cwc-infinite-list-watcher 
  onOnBottomReach={this.loadMoreUsers()}
    ...
    <cwc-list 
      ...
      />,
      <div class="btn btn-info" onClick={() => this.initUsers1Data()}>
        duplicate
      </div>,
      <cwc-list
        id="users-infinite"
